...
setup_memory_limits() {
    ...
    iptables -A FORWARD -i $users -o $isp -j M_LIMIT_IP
    iptables -A FORWARD -i $isp -o $users -j M_LIMIT_IP
    iptables -A M_LIMIT_IP -i $users -o $isp -s $IP_ADDRESS -m quota --quota $LIMIT_VALUE -j ACCEPT
...
setup_speed_limits() {
    ...
    iptables -A FORWARD -i $users -o $isp -j S_LIMIT_IP
    iptables -A FORWARD -i $isp -o $users -j S_LIMIT_IP
    iptables -A S_LIMIT_IP -i $users -o $isp -s $IP_ADDRESS -m limit --limit $LIMIT_VALUE/s -j ACCEPT
